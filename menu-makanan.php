...
<head>
    ...
    <style>
        /* Memastikan tata letak lebih rapi */
...
<body>
    ...
    <main>
        <section>
            <h2>Menu Makanan</h2>
            <p>Berbagai pilihan makanan tersedia untuk Anda.</p>
            <div class="menu-list">
                <?php
                // Koneksi ke database
                ...
                $total_items_query = "SELECT COUNT(*) as total FROM menu WHERE type = 'food'";
                $total_items_result = $conn->query($total_items_query);
                $total_items = $total_items_result->fetch_assoc()['total'];
                $total_pages = ceil($total_items / $items_per_page);

                // Mengambil data menu makanan dari database
                $sql = "SELECT * FROM menu WHERE type = 'food' LIMIT $offset, $items_per_page";
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "<p>Tidak ada menu makanan.</p>";
                }
            ...
            <div class="pagination">
                ...
                // Tombol "Previous"
                if ($page > 1) {
                    echo "<a href='menu-makanan.php?page=" . ($page - 1) . "'>Previous</a>";
                }

                for ($i = $start_page; $i <= $end_page; $i++) {
                    $active = ($i == $page) ? "class='active'" : "";
                    echo "<a href='menu-makanan.php?page=$i' $active>$i</a>";
                }

                // Tombol "Next"
                if ($page < $total_pages) {
                    echo "<a href='menu-makanan.php?page=" . ($page + 1) . "'>Next</a>";
                }
                ?>
            </div>
        </section>
